...
- New extensions must register `pi.on("session_shutdown", (event) => killOnHostTeardown(backend, event))` (the `createBackend` return value): backends are killed only on host teardown (quit, reload) and preserved across session replacement (new, resume, fork, /wt switches), where pi rebinds the loaded extension instances without re-importing them
