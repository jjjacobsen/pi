...
//
// Ported from @tmustier/pi-usage-extension (MIT,
// https://github.com/tmustier/pi-extensions), unchanged apart from imports.
// Pure and theme-free: series building works off the hourly buckets produced
// by the Zig backend, and the renderer emits plain text plus a colorize
// callback so the UI layer owns all styling.

import type { HourlyCell, PeriodBounds, TabName } from "./types";
import { splitHourlyKey } from "./types";
...
function groupKeyOf(key: string, groupBy: GraphGroupBy): string {
...
function domainFor(period: TabName, bounds: PeriodBounds, hourly: Map<number, Map<string, HourlyCell>>): { startMs: number; endMs: number } {
	switch (period) {
		case "today":
		...
		for (let i = firstIdx; i <= lastIdx; i++) {
			...
			if (prevX >= 0) {
				...
				for (let s = 1; s <= steps; s++) {
					...
					const it = Math.round(prevY + ((y - prevY) * s) / steps);
					setDot(ix, it, seriesIndex);
				}
			} else {
