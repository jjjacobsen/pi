...
// web_search tool and calls out to src/search.zig as a one-shot process:
// the request travels as one JSON argv element via pi.exec (shared callZig
// helper), the binary prints one JSON envelope to stdout and exits. All
// search logic lives in Zig: the Exa request body, the response parse,
// result formatting, and the deadline.
//
