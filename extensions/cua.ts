...
const TOOLS: { name: string; mcp?: string; description: string; params: TProperties }[] = [
  ...
  { name: "computer_zoom", mcp: "zoom", description: "Crop a window region (x1,y1)-(x2,y2) in screenshot pixels to a JPEG at most 500px wide, saved to disk with its path returned. Call describe_image on the path to read text or inspect pixels at readable size. Click/type coordinates taken from a zoom image translate back to full-window space automatically. Only use once a screenshot is already justified, never instead of the AX tree.",
