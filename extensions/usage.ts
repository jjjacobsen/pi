...
// =============================================================================
// Period bounds (local calendar, same rules as the original data.ts)
// =============================================================================

function computeBounds(): PeriodBounds {
	const now = new Date();
	...
	return {
		...
		last30DaysStartMs: startOfLast30Days.getTime(),
		nowMs: now.getTime(),
	};
}
...
interface DataColumn {
	...
	width: number;
	dimmed?: boolean;
	getValue: (stats: BaseStats & { sessions: number }) => string;
}
...
/** "3h15m", "1d13h", "45s" — same shape omp uses for quota reset timers. */
function formatDuration(ms: number): string {
	...
	const HOUR = 60 * MIN;
	const DAY = 24 * HOUR;
	if (ms < MIN) return `${Math.max(1, Math.round(ms / SEC))}s`;
	if (ms < HOUR) {
...
function accountLabel(p: ProviderLimits): string {
	const { email, planType } = p.account;
	if (email) return planType ? `${email} (${planType})` : email;
	if (p.provider === "opencode-go") return "account 1";
	if (planType) return planType;
	if (p.account.accountId) return p.account.accountId.slice(0, 8);
	return "";
}

function limitStatusColor(status: string): "success" | "warning" | "error" | "dim" {
...
function limitStatusIcon(status: string): string {
...
/** "98.4% free" — remaining fraction of a single limit, one decimal. */
function formatRemaining(remainingFraction: number | null): string {
	if (remainingFraction === null) return "—% free";
	const pct = Math.max(0, remainingFraction * 100);
	const shown = pct >= 100 ? "100" : pct.toFixed(1);
	return `${shown}% free`;
}
...
function renderUsageBar(fraction: number | null, barWidth: number, color: "success" | "warning" | "error" | "dim", theme: Theme): string {
	...
	return theme.fg(color, leading) + theme.fg("dim", empty);
}
...
class UsageComponent {
	...
	private visibleTable(): { providers: Map<string, { messages: number; cost: number; sessions: number; tokens: TotalStats["tokens"]; models: Map<string, { messages: number; cost: number; sessions: number; tokens: TotalStats["tokens"] }> }>; totals: TotalStats } {
		...
		const providers = new Map<string, { messages: number; cost: number; sessions: number; tokens: TotalStats["tokens"]; models: Map<string, { messages: number; cost: number; sessions: number; tokens: TotalStats["tokens"] }> }>();
		...
		for (const provider of providers.values()) {
			...
			totals.sessions += provider.sessions;
	...
	private renderLimits(width: number): string[] {
		...
		if (this.limitsError) {
			lines.push(th.fg("dim", `  Provider limits unavailable: ${this.limitsError}`));
			lines.push(th.fg("dim", "  [r] retries"));
			lines.push("");
			return lines;
		}
		if (!this.limits) {
			lines.push(th.fg("dim", "  Fetching provider limits…"));
			lines.push("");
			return lines;
		}

		const age = formatDuration(Date.now() - this.limits.fetchedAt);
		lines.push(th.fg("muted", `Provider limits · fetched ${age} ago`));
		lines.push("");

		for (const p of this.limits.providers) {
			lines.push(th.bold(th.fg("accent", providerDisplayName(p.provider))));
			if (p.error) {
				lines.push(`  ${th.fg("warning", "⚠ ")} ${th.fg("dim", p.error)}`);
				lines.push("");
				continue;
			}
			const acct = accountLabel(p);
			if (acct) lines.push(`  ${th.fg("dim", acct)}`);

			// Compute a shared bar width: leave room for the trailing
			...
			for (const l of p.limits) {
				const color = limitStatusColor(l.status);
				const icon = limitStatusIcon(l.status);
				const windowSuffix =
			...
			if (p.resetCredits !== null && p.resetCredits > 0) {
				lines.push(`  ${th.fg("dim", `✦ ${p.resetCredits} saved reset${p.resetCredits === 1 ? "" : "s"} available`)}`);
			}
			lines.push("");
		}
		return lines;
	...
	private renderDataRow(
		name: string,
		stats: BaseStats & { sessions: number },
		layout: TableLayout,
