...
fn detectWebp(bytes: []const u8) !ImageInfo {
    ...
    while (i + 8 <= bytes.len) {
        ...
        if (mem.eql(u8, fourcc, "VP8X")) {
            if (i + 30 > bytes.len) return error.UnsupportedFormat;
            const w = readLe24(bytes[i + 24 ..]) + 1;
            const h = readLe24(bytes[i + 27 ..]) + 1;
            if (w == 0 or h == 0) return error.UnsupportedFormat;
            return .{ .mime = "image/webp", .width = w, .height = h, .has_alpha = (bytes[i + 20] & 0x10) != 0 };
        }
...
fn compressImage(arena: Allocator, io: std.Io, src: []const u8, info: ImageInfo, max_dim: u32, quality: u8, sips_err: *[]const u8) !CompressedImage {
    ...
    return .{ .bytes = bytes, .mime = if (to_jpeg) "image/jpeg" else info.mime };
}
...
fn selfCheck(gpa: Allocator, io: std.Io) !void {
    ...
    // The server must have served exactly 5 requests (2 for the retry test,
    // 1 compression, 1 four-hundred, 1 timeout). A wrongly retried 4xx or a
    // double send would shift the count. Wait for the server to finish
    // serving (the timeout test's response lands ~2s after the client gave up)
    // before counting.
    var waited: i64 = 0;
